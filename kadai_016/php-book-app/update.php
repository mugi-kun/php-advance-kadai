...
if(isset($_POST['submit'])) {
  try{
    ...
    // 動的に変わる値をプレースホルダに置き換えたUPDATE文をあらかじめ用紙する
    $sql_update = '
    UPDATE books
...
if(isset($_GET['id'])) {
  ...
  // idパラメータの値と同じデータが存在しない場合はエラーメッセージを表示して処理を終了する
  // 補足:fetch()メソッドは実行結果が取得できたなっかた場合にFALSEを返す
  if ($book === FALSE){
    exit('idパラメータが不正です。');
  }
        ...
        <div>
          ...
          <select name="genre_code" required>
            ...
            foreach($genre_codes as $genre_code) {
              // もし変数$genre_codeが商品のジャンルコードと一致していれば、selected属性をつけて初期値にする
